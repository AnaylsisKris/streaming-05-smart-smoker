"""
    Food A Temperature Monitoring during cooking via "smart smoker"
    This program listens for Food B messages contiously. 
    If food temp change in temp is 1 degF or less in 10 min (or 21 readings)  --> food stall alert!
...
# send email alerts (unable to set up)
# requires emailer.py file
# from emailer import createAndSendEmailAlert

# changed maxlen to 21 instead of 20 after observing actual time_change
temp_deque = deque(maxlen=21)
time_deque = deque(maxlen=21)
...
def smoker_callback(ch, method, properties, body):
    ...
    try:
        ...
        # process messages with values
            ...
            temp_change = abs(temp_deque[-1] - temp_deque[0])
            ...
            # using deque, create the alert for temps > 15 deg every 5 readings
            if len(temp_deque) == 21:

                if temp_change < 1.0:
                    logger.info(f"FOOD STALL ALERT! --> food temp decreases by 1.0 F or more in 10 min (or 21 readings)")
                    logger.info(f"Temperature Change: {round(float(temp_change),1)} degrees. Time Change: {time_change} mins.")
                    logger.info("----------------------------------------")

                    # send email alerts (not set up)
                    '''
                    email_subject = "FOOD-A STALL ALERT!"
                    email_body = (f"Food-A Temperature Change: {round(float(temp_change),1)} degrees. Time Change: {time_change} mins.")
